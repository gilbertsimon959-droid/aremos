...
export const createSession = async (request: FastifyRequest<{ Body: ICreateSessionDto }>, reply: FastifyReply) => {
  try {
    ...
    const session = await prisma.learningSession.create({ data: { userId: authUser.id, deckId, duration: durationMs, interactions: { create: interactions.map(i => ({ cardId: i.cardId, round: i.round, isCorrect: i.isCorrect })) } } });
