...
export const finishPractice = async (request: FastifyRequest<{ Params: { sessionId: string }; Body: { durationMs: number } }>, reply: FastifyReply) => {
  try {
    ...
    await prisma.learningSession.create({ data: { userId: authUser.id, deckId: sess.deckId, duration: Math.max(1, Math.floor(durationMs || 0)), interactions: { create: interactionsData } } });
