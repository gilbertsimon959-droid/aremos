...
/** Helper: copy original deck (with cards & SR) to a target user if not existing */
async function copyDeckToUser(originalDeckId: number, targetUserId: number) {
  const exists = await prisma.deck.findFirst({ where: { ownerId: targetUserId, copiedFromId: originalDeckId } });
  if (exists) return;
