...
export const uploadImageAndSetOnCard = async (request: FastifyRequest<{ Params: { deckId: string; cardId: string } }>, reply: FastifyReply) => {
  try {
    const authUser = (request as any)['authUser']; if (!authUser) throw ERRORS.unauthorizedAccess;
    ...
    const allowed = ['image/jpeg','image/png','image/webp'];
    if (!file) throw new AppError('Keine Datei empfangen', 400);
    if (file._buf && file._buf.length > MAX) throw new AppError('Datei zu groß (max. 10MB)', 400);
    if (!allowed.includes(file.mimetype)) throw new AppError('Ungültiger Dateityp (nur jpg/png/webp)', 400);
    ...
    if (STORAGE_DRIVER === 'supabase') {
      ...
      const filename = `card-${cardId}-${Date.now()}.${file.mimetype.split('/')[1]}`;
    ...
    // Update card with image URL (assuming question image for now)
    await prisma.card.update({
      where: { id: cardId },
      data: { questionImageUrl: publicUrl }
    });
...
export const deleteImageFromCard = async (request: FastifyRequest<{ Params: { deckId: string; cardId: string } }>, reply: FastifyReply) => {
  try {
    const authUser = (request as any)['authUser']; if (!authUser) throw ERRORS.unauthorizedAccess;
