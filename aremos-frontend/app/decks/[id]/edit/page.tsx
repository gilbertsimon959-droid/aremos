...
export default function DeckEditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleFinishEditing = async () => {
    try {
      // Speichere alle Änderungen und erstelle neue Deck-Version
      // Gemäß Spezifikation: Jede Speicherung erzeugt eine neue Version
      const response = await api<{ newVersion: number }>(`/api/decks/${resolvedParams.id}/create-version`, {
        method: "POST",
        body: JSON.stringify({
          ...
          spacedRepetitionDates: Array.from(selectedDates)
        })
      })
      
      alert(`Deck erfolgreich aktualisiert! Neue Version: ${response.newVersion}`)
      router.push("/decks")
    } catch (error) {
  ...
  const uploadImage = async (file: File) => {
    ...
    if (file.size > maxSize) {
      logUploadError(new Error("Datei zu groß"), "Bild-Upload", true)
      return
    }
    
    if (!allowedTypes.includes(file.type)) {
      logUploadError(new Error("Ungültiger Dateityp"), "Bild-Upload", true)
      return
    }
    ...
    try {
      ...
      // Upload zu Server
      const uploadResponse = await api<{ url: string }>('/api/uploads/image', {
        method: 'POST',
        body: formData
      })
      
      // Verknüpfe Bild mit Karte
      const currentCard = cards[currentCardIndex]
