  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const displayMessage = ERROR_MESSAGES[errorMessage] || errorMessage
        
        // Map errors to specific fields
        if (errorMessage.includes('Name') || errorMessage.includes('Benutzer nicht gefunden')) {
          setFormErrors({ name: displayMessage })
        } else if (errorMessage.includes('Lizenzcode')) {
          setFormErrors({ licenseCode: displayMessage })
        } else if (errorMessage.includes('Passwort')) {
          setFormErrors({ password: displayMessage })
        } else {
          setFormErrors({ general: displayMessage })
        }
      }
    } catch (error) {
